...
<head>
    ...
    <title>Admin - Open Court Report</title>
    ...
    function drawCharts() {
        fetch('./database/getchar_opencourt.php?venue=opencourt')
            ...
            .then(data => {
                ...
                var monthlyOptions = {
                    title: 'Open Court Bookings (Monthly)',
                    hAxis: {title: 'Date'},
                    vAxis: {title: 'Number of Bookings'}
                };

                var monthlyChart = new google.visualization.ColumnChart(document.getElementById('opencourt_monthly_chart'));
                ...
                var weeklyOptions = {
                    title: 'Open Court Bookings (Weekly)',
                    hAxis: {title: 'Date'},
                    vAxis: {title: 'Number of Bookings'}
                };

                var weeklyChart = new google.visualization.ColumnChart(document.getElementById('opencourt_weekly_chart'));
                weeklyChart.draw(weeklyData, weeklyOptions);

                // Status Chart
                var statusData = google.visualization.arrayToDataTable([
                    ['Status', 'Count'],
                    ['ACCEPTED', data.dailyReport.ACCEPTED || 0],
                    ['DENIED', data.dailyReport.DENIED || 0],
                    ['CANCELLED', data.dailyReport.CANCELLED || 0]
                ]);

                var statusOptions = {
                    title: 'Open Court Booking Statuses (Monthly)',
                    pieHole: 0.4,
                };

                var statusChart = new google.visualization.PieChart(document.getElementById('opencourt_status_chart'));
    ...
    <div class="wrapper">
        ...
        <div class="main_body">
            ...
            <div class="report weekly_report">
                <h3>Weekly Report</h3>
                <div id="opencourt_weekly_chart" style="width: 100%; height: 500px;"></div>
            </div>
